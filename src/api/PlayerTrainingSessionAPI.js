 ...
 const getPlayerTrainingSession = async (playerID, trainingSessionID) => {
    try {
        return (await DataStore.query(PlayerTrainingSession)).filter(
            (playerTS) => (playerTS.trainingsessionID === trainingSessionID &&
...
const getPlayerTrainingSessionFromTS = async (tsID) => {
    ...
    try {
        return (await DataStore.query(PlayerTrainingSession)).filter(
            (playerTS) => (playerTS.trainingsessionID === tsID && playerTS.Deleted == false)
        );
    } catch (error) {
        console.log(error);
    }
 ...
 */
 const getPlayerNames = async (trainingSessionID) => {
    //let names=[]
    try {
        const playerid= await getTrainingSessionPlayerTrainingSessions(trainingSessionID)
        let names=[]
 ...
 */
 const getPlayerTrainingSessionsByTSandKitID = async (trainingSessionID, kitID) => {
    try {
        return (await DataStore.query(PlayerTrainingSession)).filter(
            (playerTS) => playerTS.trainingsessionID === trainingSessionID &&
...
const PlayerTrainingSessionAPI = {
    ...
    getTrainingSessionPlayerTrainingSessions: getTrainingSessionPlayerTrainingSessions,
